...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    const codeArr = str.split('').map((item) => item.charCodeAt());
    return codeArr.reduce((accu, item) => accu + item, 0);
  }
  return 0;
}
...
function isPalindrome(str) {
  const modifyStr = str.toLowerCase().replace(/[^A-Za-z0-9]/g, '');
  const polidrome = modifyStr.split('').reverse().join('');

  return modifyStr === polidrome;
}
